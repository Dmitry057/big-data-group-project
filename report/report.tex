...
fact table four different ways, then measure the on-disk footprint
(via \texttt{hadoop fs -du}) and time a full scan and one analytic
...
\toprule
\textbf{Format} & \textbf{Codec} & \textbf{Runs} & \textbf{Size (B)} & \textbf{Avg write (s)} & \textbf{Avg full scan (s)} & \textbf{Avg analytic (s)} \\
\midrule
parquet & snappy & 3 & 4{,}506{,}447 & 17.7 & 6.76 & 9.29 \\
parquet & gzip   & 3 & 3{,}003{,}399 & 18.0 & 6.64 & 9.56 \\
avro    & snappy & 3 & 4{,}941{,}651 & 17.0 & 9.32 & 10.50 \\
avro    & bzip2  & 3 & 3{,}082{,}133 & 19.0 & 8.36 & 11.01 \\
\bottomrule
\end{tabular}
\caption{Storage format benchmark. Parquet beats Avro on scan time by
a clear margin. On size, gzip and bzip2 shave roughly $33\%$ off Snappy
--- a real saving on disk but at a CPU cost on every read; Snappy keeps
scans the fastest at a sensible footprint, so we shipped Parquet +
Snappy. Raw numbers in
\texttt{output/stage1\_storage\_benchmark\_avg.csv}.}
\end{table}
...
\caption{Layer 2 in action --- \texttt{bash scripts/run\_tests.sh} on
the cluster from \texttt{team30@hadoop-01}. The 15 cloud-safe unit
tests pass, and 13 of the 20 cluster smoke tests run green; the 7
\texttt{skip} lines are correct \texttt{require\_cmd}/
\texttt{require\_secret} behaviour --- \texttt{psql} is not installed
on the edge node and \texttt{DASHBOARD\_URL} is unset for this run.
The smoke suite covers HDFS warehouse, Hive table inventory + partition
layout, the \texttt{q1\_results} row-count check against the committed
\texttt{output/q1.csv}, and the Spark \texttt{ml\_evaluation.py} re-run
on persisted predictions.}
\end{figure}
