...
(repo-structure + artefact-schema invariants, regression guards on
\texttt{report.tex}). \\
\midrule
...
\begin{itemize}[leftmargin=2em]
    ...
    These tests assert that the right files exist on disk, that the
    committed \texttt{output/evaluation.csv} still has Random Forest at
    F1 $\geq 0.99$, and that \texttt{report.tex} doesn't regress to old
    values --- the raw CSV size stays at ``3.39\,GB'' and no stale Citus
...
schema check, the Random-Forest F1 regression guard, and the three
\texttt{report.tex} regressions (no leftover ``Citus'', the raw CSV
size is pinned at ``3.39\,GB'', and the new repo URL is in place).}
\end{figure}

\subsection{What it caught}

Three concrete regressions during the project would have been merged
without CI:

\begin{itemize}[leftmargin=2em]
    \item An early \texttt{report.tex} commit understated the raw CSV
    size; the
    \texttt{tests/unit/02\_artifacts\_schema.bats} regression guard now
    pins the value at \texttt{$\sim$3.39\,GB} and blocks any wrong
    figure from sneaking back in.
    \item A draft of \texttt{pipeline.svg} still showed \texttt{CITUS
    $\cdot$ TEAM30\_PROJECTDB} after we'd dropped Citus from the stack;
    the same guard greps \texttt{report.tex} (and could trivially be
    extended to \texttt{pipeline.svg}).
