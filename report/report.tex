...
\begin{itemize}[leftmargin=2em]
    ...
    values (no leftover ``Citus'', no ``737\,MB'' instead of
    ``3.39\,GB'').
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
    \item An early \texttt{report.tex} commit still claimed
    \texttt{$\sim$737\,MB} for the raw CSV size. The
    \texttt{tests/unit/02\_artifacts\_schema.bats} regression guard
    blocks that string from sneaking back in.
